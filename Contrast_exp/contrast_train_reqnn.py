...
from contrast_model_reqnn import ContrastPointNetREQNN
...
def parse_args():
    ...
    p.add_argument('--exp_name', type=str, default='REQNNTest', help='')
    p.add_argument('--save_dir', help='日志保存路径', default='/data/home/scvi197/run/cjj/pointnet.pytorch/Contrast_exp/experiment/reqnn', type=str)
    return p.parse_args()
...
@torch.no_grad()
def evaluate(net: ContrastPointNetREQNN, loader, device):
    net.eval()
    correct, total = 0, 0
...
def main():
    ...
    num_classes = 40
    net = ContrastPointNetREQNN(k=num_classes).to(device)
    ...
    for epoch in range(args.nepoch):
        ...
        for batch_data in train_pbar:
            ...
            # two viewspred, final_quat_vec, local_feat
            logp1, final_quat_vec1, local_feat1 = net(v1)
            logp2, final_quat_vec2, local_feat2 = net(v2)
            ...
            con = info_nce_loss(local_feat1, local_feat2, temperature=args.temperature)

            # region contrastive loss
            # region_con = 0.0
            if args.use_region_contrast and segments is not None:
                # 使用真正的点级特征 local_feat1, local_feat2 (B, 1024, N)
                region_con = region_contrastive_loss(local_feat1, local_feat2, segments, temperature=args.temperature)

            # total loss
            loss = ce + args.lambda_con * con + args.lambda_region * region_con
            loss.backward()
            optimizer.step()
